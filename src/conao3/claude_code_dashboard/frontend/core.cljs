...
(defn ProjectList []
  (let [result (apollo.react/useQuery projects-query)
    ...
    (cond
      ...
      error [:p.text-negative-color-900 (str "Error: " (.-message error))]
      :else [:ul.space-y-2
             (for [edge (-> data .-projects .-edges)]
               (let [node (.-node edge)]
                 [:li.p-3.bg-background-layer-2.rounded.text-neutral-content-color.hover:bg-gray-200.cursor-pointer
                  {:key (.-id node)}
                  (.-name node)]))])))

(defn App []
  [:> apollo.react/ApolloProvider {:client apollo-client}
   [:div.min-h-screen.bg-background-base.text-neutral-content-color.p-8
    [:h1.text-3xl.font-bold.mb-6.text-accent-content-color "Claude Code Dashboard"]
    [:f> ProjectList]]])
